...
public class UpdateListingValidator : AbstractValidator<UpdateListingDto>
{
    public UpdateListingValidator()
    {
        ...
        RuleFor(x => x.Description).NotEmpty().MaximumLength(140);
        RuleFor(x => x.Amenities).NotEmpty().MaximumLength(140);
        RuleFor(x => x.HouseRules).NotEmpty().MaximumLength(100);
        RuleFor(x => x.Pricing).GreaterThanOrEqualTo(0);
        RuleFor(x => x.BedCount).GreaterThan(0);
        RuleFor(x => x.BathCount).GreaterThan(0);
        RuleFor(x => x.PropertyType).NotEmpty().MaximumLength(20);
